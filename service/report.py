...
—— 本次改动（版式优化配套）——
1. METRICS 每项新增「参考区间」和「低于/高于区间时的说法」，并算出 out/state；
   条形图不再只表示占量程比例，而是叠一条灰色参考带 + 读数标记。
2. 有符号的偏移量（水平/垂直）改画双向轴：0 在正中，负数向左、正数向右，
   显示满程 ±DISPLAY_SPAN，参考带对称跨在中间。
3. 判级配色和越界配色一起放在上下文里，模板不写死颜色。
"""
...
#   视野和曝光。2026-08-28 用 30 张真实炉膛照片的 P10~P90 重标过；此前是
#   拍脑袋的值，跟实际取景完全对不上，四项指标对每张图都报超限，等于没信息量。
#   这类区间的含义是「相对本机位是否异常」，换个摄像头要重新标。
#
#   物理意义明确的（贴壁亮斑、边缘带高温）—— 不按频率标。这批素材里贴壁亮斑
#   很常见，但「常见」不等于「正常」，按 P90 放宽会把最该报警的信号压掉。
#   这两项维持严格阈值。
METRICS = [
    ("flame_fill_ratio_pct",      "火焰充满度",         "%", 100, (12, 34),    "偏低", "偏高", False),
    ("centroid_offset_total_pct", "火焰中心偏斜",       "%", 100, (0, 30),     "",     "偏大", False),
    ("centroid_offset_x_pct",     "水平偏移（负=偏左）", "%", 100, (0, 25),     "偏左", "偏右", True),
    ("centroid_offset_y_pct",     "垂直偏移（负=偏上）", "%", 100, (0, 25),     "偏上", "偏下", True),
    ("temp_index_mean",           "相对亮温均值",       "",  1,   (0.70, 0.80), "偏低", "偏高", False),
    ("temp_index_p95",            "相对亮温 P95",       "",  1,   (0.78, 0.88), "偏低", "偏高", False),
    ("high_temp_area_pct",        "高温区占比",         "%", 100, (20, 75),    "偏小", "偏大", False),
    ...
    ("wall_hotspot_area_pct",     "贴壁亮斑面积",       "%", 100, (0, 0.3),    "",     "偏高", False),
]
